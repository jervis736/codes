    ...
    while (sfRenderWindow_pollEvent(window, &event)) {
        if (event.type == sfEvtMouseButtonPressed) {
            ...
            if (position.x > sprite_position.x && position.x < sprite_position.x + 110 &&
                ...
                Soundbuffer = sfSoundBuffer_createFromFile("src/dio-wryyy.ogg");
...
int display_background_sound(sfRenderWindow *window, sfSprite *sprite, sfSprite *sprite_2)
{
    ...
    //crée la texture en me référencent au txt de la map, et faire défiler cette dite texture.
    //texture = sfTexture_createFromFile("src/jojo_bg.jpg", NULL);
    texture_2 = sfTexture_createFromFile("src/canard.png", NULL);
    sfSprite_setTexture(sprite, texture, sfTrue);
    sfSprite_setTexture(sprite_2, texture_2, sfTrue);
    Soundbuffer = sfSoundBuffer_createFromFile("src/jojoOp1.ogg");
